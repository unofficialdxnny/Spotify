...
class Gen:
    def __init__(self):
        ...
        self.config_file = json.load(open('config.json', 'r', encoding='utf-8'))
        ...
        if len(self.proxies) == 0 and self.settings['Use_Proxy'] == 'y':
            ...
            self.console.printe('Continuing without using proxy because there is no proxy in proxies.txt folder.')
            sleep(1)
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if self.save_methods['Text_File'] == 'y':
                        with open('accounts_mail.txt', 'a', encoding='utf-8') as f:
                            f.write(f"{mail}\n")
                        with open('accounts_password', 'a', encoding="utf-8") as f:
                            f.write(f"{password}\n")
